...
int SimplePass::checkPreconditions(SquadPosition* player, Stadium* stadium)
{
	...
	// Look for simple pass opportunities
	...
	for (int i=0; i<11; i++)
	{
		...
		if (target!=player && distance>2 && distance<visionDistance && !otherClub->isSomeoneThere_points(player->getPosition(),target->getPosition(),2))
		{
			...
			possibilities.insert( std::pair<float,int>(random->get_float(-1.25,-0.75)*howAdvancedTargetIs/distance,i) );
		}
	}
...
bool SimplePass::execute(SquadPosition* player, Stadium* stadium)
{
	...
	float sd = 0.18 - (0.15 * player->get_footballer()->shortPassing * 0.01);
	...
	stadium->ball->applyImpulse(sf::Vector3f(new_impulse.x,random->get_float(1.0,1.9),new_impulse.y),sf::Vector3f(0,0,0));
	stadium->lastTeamTouchedTheBall = player->club->isAway;

	stadium->commentator = player->get_footballer()->fullName_shown + language->ACTION_00002;
